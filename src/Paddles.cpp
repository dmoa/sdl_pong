...
void Paddles::Update(double dt, const Uint8* keys_down, Ball* ball) {
    ...
    int ball_left_screen = ball->LeftScreen();
    if (ball_left_screen == 1) {
        m_paddle2.score++;
    } else if (ball_left_screen == -1) {
        m_paddle1.score++;
    }
}

int Paddles::GetScore1() {
    return m_paddle1.score;
}

int Paddles::GetScore2() {
    return m_paddle2.score;
}